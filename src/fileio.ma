...
2$:	0
	...
	TAD	(LCLFIB+FILLIN	/SET UP POINTER TO FILEBLOCK
	DCA	X0
...
OOPEN1,	0
	...
	RTL;RTL
	AND	(7760
	TAD	LCLFIB+DEVNUM	/COMBINED WITH DEVICE #
	CIF	10
	JMS	USR	/CALL USR
	3		/ENTER
1$:	SBFILE		/ Parse puts name here
...
/
PUT1,	0
	ISZ	LCLFIB+FILBLK	/BUMP LAST.
	TAD	AC	/GET REQUESTED BLOCK.
	SZA		/ZERO?
	DCA	LCLFIB+FILBLK	/NO. SAVE AS DEFAULT.
	CMA
	TAD	LCLFIB+FILBLK	/GET LCLFIB TO WRITE.
	TAD	LCLFIB+FILBEG
	DCA	OBLK	/SAVE AS BLOCK TO OUTPUT TO.
	TAD	LCLFIB+FILEND	/GET FILE END POINTER...
	CIA
	CLL CML
	TAD	OBLK	/COMPARE WITH FILE PTR...
	SNL CLA		/L=0 PTR OUT OF FILE SPACE.
	JMP	ERR1	/YES. ERROR!
	TAD	LCLFIB+BUFADR	/NO. GET BLOCK ADDRESS
	DCA	OADR	/SAVE AS ADDRESS TO OUTPUT FROM.
	TAD	LCLFIB+BUFFLD	/GET FIELD
...
OBLK,	0
	...
	TAD	LCLFIB+FILFLG	/CLEAR MODIFIED FLAG.
	AND	(3777
	DCA	LCLFIB+FILFLG
	TAD	LCLFIB+FILBLK	/GET WRITTEN BLOCK.
	CIA CLL CML
	TAD	LCLFIB+FILSIZ	/COMPARE WITH SIZE.
	SNL		/SIZE GROW?
	JMP	1$	/NO.
	CIA		/YES. ADD CHANGE.
	TAD	LCLFIB+FILSIZ
	DCA	LCLFIB+FILSIZ	/SAVE AS NEW SIZE.

1$:	CLA
	TAD	LCLFIB+FILBLK	/GET AC.
...
GET1,	0
	...
	TAD	AC	/YES. SAVE AC.
	DCA	ACS
	TAD	LCLFIB+FILBLK	/SET UP FOR PUT TO OLD BLOCK.
	DCA	AC
	JMS	PUT1	/PUT BLOCK
	TAD	ACS	/RESTORE AC
	DCA	AC
...
2$:	ISZ	LCLFIB+FILBLK	/BUMP DEFAULT.
	TAD	AC	/GET AC.
	SZA		/ZERO?
	DCA	LCLFIB+FILBLK	/NO. SAVE AS DEAFULT.

	CMA		/NO. CALCULATE ABSOLUTE ADDRESS.
	TAD	LCLFIB+FILBLK
	TAD	LCLFIB+FILBEG
	DCA	IBLK	/SAVE IT.
	TAD	LCLFIB+FILEND	/GET FILE END PTR.
	CIA
	CLL CML
...
NXTBLK,	0
	CLA
	TAD LCLFIB+FILBLK	/GET LAST BLOCK.
	DCA AC	/SAVE AS AC FOR PUT ROUTINE.
	TAD LCLFIB+FILFLG	/BUFFER MODIFIED?
	SPA CLA
	JMS PUT1	/PUT BLOCK.
	ISZ AC		/SET AC TO READ BLOCK
	TAD LCLFIB+FILEND /GET END PTR.
	CIA
	CLL CML
	TAD AC		/GET BLOCK TO READ.
	TAD LCLFIB+FILBEG	/ADD ADDRESS.
	...
	DCA LCLFIB+BUFCNT /CLEAR OFFSET.
	TAD AC		/GET CURRENT BLOCK.
	DCA LCLFIB+FILBLK /SAVE CURRENT BLOCK.
...
1$:	TAD	LCLFIB+BUFCNT	/GET COUNT
	AND	(1774)
	CLL RAR
	TAD	LCLFIB+BUFADR
	DCA	ARG		/THIS IS POINTER TO DOUBLEWORD CONTANING CHAR
	TAD	LCLFIB+BUFFLD	/MAKE FIELD CHANGE CODE.
	TAD	(CDF)
	DCA	.+1
	HLT			/CHANGE FIELD.
	TAD	LCLFIB+BUFCNT
...
OCRET,	CDF	.	/RESTORE FIELD.
	ISZ	LCLFIB+BUFCNT	/BUMP POINTER.
	TAD	LCLFIB+FILFLG	/SET BUFFER MODIFIED BIT.
	RAL
	CLL CML RAR
	DCA	LCLFIB+FILFLG
	TAD	LCLFIB+BUFCNT	/CHECK IF BUFFER IS FULL.
	TAD	(-1000)
	SZA CLA
	JMP	1$		/IS NOT.
	TAD	LCLFIB+FILBLK	/IT IS. GET CURRENT BLOCK.
	DCA	AC
...
OCHAR,
ICHAR1,	0
	TAD	LCLFIB+BUFCNT	/CHECK BUFFER COUNT.
	TAD	(-1000)		/END?
	SZA CLA
	JMP	1$		/NO.
	JMS	NXTBLK		/YES.
...
1$:	TAD	LCLFIB+BUFCNT	/GET BUFFER COUNT.
	AND	(1774)
	CLL RAR
	TAD	LCLFIB+BUFADR
	/SAVE. THIS IS POINTER TO DOUBLEWORD CONTAINING CHARS
	DCA	ARG
	TAD	LCLFIB+BUFFLD	/GET FIELD
...
CLOSE1,	0
	...
	JMP	1$	/NO.
	TAD	LCLFIB+FILBLK	/YES. OUTPUT IT.
	DCA	AC
	JMS	PUT1

1$:	TAD	LCLFIB+FILFLG	/GET FLAGS.
	RAR
...
2$:	TAD I	X0
	...
	TAD	LCLFIB+FILSIZ	/YES. SET UP FINAL SIZE.
	DCA	FSIZ

	TAD	LCLFIB+DEVNUM	/GET DEV. #
	CIF	10
...
ECLOSE,	DCA	LCLFIB+FILFLG	/CLEAR FILE FLAGS.
	JMP I	CLOSE1	/RETURN
/
	PAGE
	.SBTTL	FLUSH
...
FLUSH1,	0		/ENTRY TO FLUSH.
	DCA	AC	/CLEAR AC.
	TAD	LCLFIB+FILFLG	/CHECK IF BLOCK HAS BEEN MODIFIED.
	SMA CLA
	JMP I	FLUSH1		/WAS NOT...

1$:	TAD	LCLFIB+BUFCNT	/FILL REST OF CURRENT WORD...
...
2$:	TAD	LCLFIB+BUFCNT	/GET OFFSET.
	...
	TAD	LCLFIB+BUFFLD	/GET BUFFER FIELD.
	TAD	(CDF)
	DCA	.+1
	HLT			/CHANGE FIELD.
...
4$:	CDF	.		/CHANGE FIELD BACK.
	...
	DCA	LCLFIB+BUFCNT
	TAD	LCLFIB+FILBLK	/GET CURRENT BLOCK #.
	DCA	AC
	JMS	PUT1		/OUTPUT CURRENT BLOCK #.
